...
namespace uGUIHelper
{
	...
	//		punch
		...
		[SerializeField][HideInInspector]
		private ProcessType m_AlphaProcessType = ProcessType.Ease ;
		public  ProcessType   alphaProcessType
		{
			get
			{
		...
		[SerializeField][HideInInspector]
		private EaseType m_AlphaEaseType = EaseType.linear ;
		...
		private Action<string, UITween> m_OnFinishedAction ;
		...
		public void SetOnFinished( Action<string, UITween> tOnFinishAction )
		{
		...
		private void DisableInteraction( CanvasGroup tCanvasGroup )
		{
...
//			if( tCanvasGroup.blocksRaycasts == false )
			if( tCanvasGroup.interactable == false )
			{
				// 最初から無効化されていたら処理しない
				return ;
			}

			// 同じ gameObject にアタッチされた全ての UITween を取得し、既にに処理が行われていないか確認する。
			...
			if( tTweenList != null && tTweenList.Length >  0 )
			{
				...
				for( i  = 0 ; i <  l ; i ++ )
				{
					if( tTweenList[ i ] != this )
					{
						if( tTweenList[ i ].m_KeepInteractionDisableProcess == true )
						{
							// 既に処理が行われている
							return ;
						}
			...
			// ブロックレイキャストを無効にする
//			tCanvasGroup.blocksRaycasts = false ;
			tCanvasGroup.interactable = false ;
		}

		private void EnableInteraction( CanvasGroup tCanvasGroup )
		{
			// 自身は処理の担当ではない
			...
			// ブロックレイキャストを有効にする
//			tCanvasGroup.blocksRaycasts = true ;
			tCanvasGroup.interactable = true ;
		...
		public void Play( float tDelay = -1, float tDuration = -1, float tOffset = 0, Action<string,UITween> onFinishedAction = null )
		{
			enabled = true ;
		...
		private void Modify( float tFactor, bool tFinished )
		{
			if( m_RectTransform != null )
			{
				if( m_PositionEnabled == true )
				{
					...
					{
						m_RectTransform.anchoredPosition = m_BasePosition + tDelta ;

						if( tFinished == true && m_ValueType == ValueType.RelativeAndUpdate && m_View != null )
						{
							m_View.localPosition = m_BasePosition + tDelta ;
						}
					}
					else
					{
						m_RectTransform.anchoredPosition = tDelta ;
					}
				}
				...
				if( m_RotationEnabled == true )
				{
					...
					if( m_ValueType == ValueType.Relative || m_ValueType == ValueType.RelativeAndUpdate )
					{
						m_RectTransform.localEulerAngles = m_BaseRotation + tDelta ;

						if( tFinished == true && m_ValueType == ValueType.RelativeAndUpdate && m_View != null )
						{
							m_View.localRotation = m_BaseRotation + tDelta ;
						}
					}
					else
					{
						m_RectTransform.localEulerAngles = tDelta ;
					}
				}
				...
				if( m_ScaleEnabled == true )
				{
					...
					if( m_ValueType == ValueType.Relative || m_ValueType == ValueType.RelativeAndUpdate )
					{
						m_RectTransform.localScale = new Vector3( m_BaseScale.x * tDelta.x, m_BaseScale.y * tDelta.y, m_BaseScale.z * tDelta.z ) ;

						if( tFinished == true && m_ValueType == ValueType.RelativeAndUpdate && m_View != null )
						{
							m_View.localScale = new Vector3( m_BaseScale.x * tDelta.x, m_BaseScale.y * tDelta.y, m_BaseScale.z * tDelta.z ) ;
						}
					}
					else
					{
						m_RectTransform.localScale = tDelta ;
					}
				}
			}
			...
			if( m_CanvasGroup != null )
			{
				...
				{
					float tDelta = GetValue( m_AlphaFrom, m_AlphaTo, tFactor, m_AlphaProcessType, m_AlphaEaseType, m_AlphaAnimationCurve ) ;
				
					if( m_View == null )
					{
						if( m_ValueType == ValueType.Relative )
						{
							m_CanvasGroup.alpha = m_BaseAlpha * tDelta ;
						}
						else
						{
							m_CanvasGroup.alpha = tDelta ;
						}
					}
					else
					{
						if( m_ValueType == ValueType.Relative )
						{
							m_CanvasGroup.alpha = m_BaseAlpha * tDelta * ( m_View.visible == true ? 1 : 0 ) ;
						}
						else
						{
							m_CanvasGroup.alpha = tDelta * ( m_View.visible == true ? 1 : 0 ) ;
						}
		...
		{
			RectTransform tRectTransform = gameObject.GetComponent<RectTransform>() ;
			if( tRectTransform != null )
			{
				m_InitialPosition = tRectTransform.anchoredPosition ;
				m_InitialRotation = tRectTransform.localEulerAngles ;
				m_InitialScale    = tRectTransform.localScale ;
			}

			CanvasGroup tCanvasGroup = gameObject.GetComponent<CanvasGroup>() ;
			if( tCanvasGroup != null )
			{
				m_InitialAlpha = tCanvasGroup.alpha ;
			}
		}
		...
		private void LoadState()
		{
			...
			if( tRectTransform != null )
			{
				tRectTransform.anchoredPosition = m_InitialPosition ;
				tRectTransform.localEulerAngles = m_InitialRotation ;
				tRectTransform.localScale       = m_InitialScale ;
			}

			CanvasGroup tCanvasGroup = gameObject.GetComponent<CanvasGroup>() ;
			if( tCanvasGroup != null )
			{
				tCanvasGroup.alpha = m_InitialAlpha ;
			}
		}

		// デバッグ用
		private void SetCheckFactor( float tCheckFactor, Vector3 tInitialPosition, Vector3 tInitialRotation, Vector3 tInitialScale, float tInitialAlpha )
		{
			m_RectTransform	= gameObject.GetComponent<RectTransform>() ;
			m_CanvasGroup	= gameObject.GetComponent<CanvasGroup>() ;
