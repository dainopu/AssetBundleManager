...
namespace UnityEngine.UI
{
	...
	/// </summary>
	[AddComponentMenu("UI/Input Field Plus")]
		...
		ILayoutElement
	{
		...
		//-----------------------------------------------------------
		// 追加機能
		[FormerlySerializedAs("imeColor")]
		[SerializeField]
		private Color m_ImeColor = Color.blue ;
		//-----------------------------------------------------------
		...
		//-----------------------------------------------------------
		// 追加機能
		private TextColorModifier	m_TextColorModifier = null ;
		//-----------------------------------------------------------
...
//					case RuntimePlatform.TizenPlayer :
		...
		public string text
		{
			...
			set
			{
				...
				if( m_CaretSelectPosition >  m_Text.Length )
				{
		...
		public float caretBlinkRate
		{
			...
			set
			{
				if( SetStruct( ref m_CaretBlinkRate, value ) )
				{
					if( m_AllowInput )
					{
						SetCaretActive() ;
		...
		public int caretWidth { get { return m_CaretWidth ; } set { if( SetStruct(ref m_CaretWidth, value)) MarkGeometryAsDirty() ; } }
		...
		public Graphic placeholder { get { return m_Placeholder; } set { SetClass(ref m_Placeholder, value); } }

		public Color caretColor { get { return customCaretColor ? m_CaretColor : textComponent.color; } set { if ( SetColor(ref m_CaretColor, value)) MarkGeometryAsDirty(); } }
		...
		public Color selectionColor { get { return m_SelectionColor; } set { if ( SetColor(ref m_SelectionColor, value)) MarkGeometryAsDirty(); } }

		public SubmitEvent onEndEdit { get { return m_OnEndEdit; } set { SetClass(ref m_OnEndEdit, value); } }

		[Obsolete("onValueChange has been renamed to onValueChanged")]
		public OnChangeEvent onValueChange { get { return onValueChanged; } set { onValueChanged = value; } }

		public OnChangeEvent onValueChanged { get { return m_OnValueChanged; } set { SetClass(ref m_OnValueChanged, value); } }

		public OnValidateInput onValidateInput { get { return m_OnValidateInput; } set { SetClass(ref m_OnValidateInput, value); } }

		public int characterLimit { get { return m_CharacterLimit; } set { if ( SetStruct(ref m_CharacterLimit, Math.Max(0, value))) UpdateLabel(); } }

		// Content Type related

		public ContentType contentType { get { return m_ContentType; } set { if ( SetStruct(ref m_ContentType, value)) EnforceContentType(); } }
		...
		public InputType inputType { get { return m_InputType; } set { if ( SetStruct(ref m_InputType, value)) SetToCustom(); } }
		...
		public CharacterValidation characterValidation { get { return m_CharacterValidation; } set { if ( SetStruct(ref m_CharacterValidation, value)) SetToCustom(); } }
		...
		public char asteriskChar { get { return m_AsteriskChar; } set { if ( SetStruct(ref m_AsteriskChar, value)) UpdateLabel(); } }
		...
		protected override void OnEnable()
		{
			...
			if( m_TextComponent != null )
			{
				...
				//---------------------------------
				// 機能追加
				m_TextColorModifier = m_TextComponent.GetComponent<TextColorModifier>() ;
				//---------------------------------
...
//				if( m_Keyboard.wasCanceled )
		...
		private bool IsValidChar( char c )
		{
			...
			// Unity2018.2 以降のバグ対応(存在するフォントまで存在しないと返される)
//			return m_TextComponent.font.HasCharacter( c ) ;
		...
		private string m_CompositionString = "" ;
		//--------------------------------------------------------
		...
		public virtual void OnUpdateSelected( BaseEventData eventData )
		{
            ...
            string fixedCompositionString = null ;
            int position = 0 ;
            if( m_CompositionString != compositionString )
            {
                if( string.IsNullOrEmpty( m_CompositionString ) == false && string.IsNullOrEmpty( compositionString ) == true )
                {
                    // IME入力文字列決定
                    fixedCompositionString = m_CompositionString ;
                }
			...
			while( Event.PopEvent( m_ProcessingEvent ) )
			{
				if( m_ProcessingEvent.rawType == EventType.KeyDown )
				{
                    ...
                    {
                        // Windows:ＩＭＥ入力完了時に２重に入力されるバグに対処する
                        if( m_ProcessingEvent.character != 0 && string.IsNullOrEmpty(fixedCompositionString ) == false )
                        {
                            if( position >= fixedCompositionString.Length || fixedCompositionString[ position ] != m_ProcessingEvent.character )
                            {
                                continue ;   // 無視
                            }
                            position ++ ;
                    ...
                    {
                        // Machintosh:決定キーだと全角文字が入力されないバグに対処する
                        if( m_ProcessingEvent.keyCode == KeyCode.Return || m_ProcessingEvent.keyCode == KeyCode.KeypadEnter )
                        {
                            if( string.IsNullOrEmpty(fixedCompositionString ) == false )
                            {
                                Append( fixedCompositionString ) ; // 文字列追加
		...
		protected void UpdateLabel()
		{
			if( m_TextComponent != null && m_TextComponent.font != null && !m_PreventFontCallback )
			{
				...
				//--------------------------------
				// 機能追加
				int		tModifierIndex = 0 ;
				Color[] tModifierColor = null ;
				//---------------------------------
				...
				if( compositionString.Length >  0 )
				{
					...
					//--------------------------------
					// 機能追加
					if( m_TextColorModifier != null )
					{
						tModifierIndex = m_CaretPosition ;
						int i, l = compositionString.Length ;
						tModifierColor = new Color[ l ] ;
						for( i  = 0 ; i <  l ; i ++ )
						{
							tModifierColor[ i ] = m_ImeColor ;
						}
					}
					//--------------------------------
				}
				...
				if( m_TextColorModifier != null )
				{
					m_TextColorModifier.index = tModifierIndex - m_DrawStart ;
					m_TextColorModifier.color = tModifierColor ;
				}
				//---------------------------------
		...
		{
#if UNITY_EDITOR
			if( Application.isPlaying == false || UnityEditor.PrefabUtility.GetPrefabInstanceHandle( gameObject ) != null )
			{
				return ;
			}
#endif
		...
		private bool SetColor(ref Color currentValue, Color newValue)
		{
